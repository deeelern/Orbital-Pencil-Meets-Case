...
function MultiSelectRow({ options, value, onChange, exclusiveOptions = [] }) {
  ...
  const handleOptionPress = (option) => {
    ...
    const hasExclusive = currentSelections.some(item => exclusiveOptions.includes(item));
    ...
    onChange(newSelections.length > 0 ? newSelections : [exclusiveOptions[0] || options[0]]);
  };
      ...
      {options.map(opt => (
        ...
        >
          <Text style={[
            ...
            currentSelections.includes(opt) && styles.multiSelectLabelSelected
          ]}>
            {opt}
          </Text>
        </TouchableOpacity>
...
export default function MyPreferencesScreen({ navigation, route }) {
  ...
  const ethnicOpts = ['All','Asian','Black/African','Hispanic/Latino','Middle Eastern','White','Indigenous','Other'];
  const relOpts = ['All','None','Buddhist','Christian','Catholic','Hindu','Jewish','Muslim','Sikh','Taoist','Atheist','Other'];
  const goalOpts = ['All','Friendship','Short-term fun','Long-term','Marriage'];
  ...
  useEffect(() => {
    if (isEditing && uid) {
      const fetch = async () => {
        ...
        if (prefs.ethnicity) setEthnicity(Array.isArray(prefs.ethnicity) ? prefs.ethnicity : [prefs.ethnicity]);
        if (prefs.religion) setReligion(Array.isArray(prefs.religion) ? prefs.religion : [prefs.religion]);
  ...
  const handleDone = async () => {
    ...
    if (ageMin > ageMax) return Alert.alert('Age range error', 'Min age cannot exceed Max age.');
    if (heightMin > heightMax) return Alert.alert('Height range error', 'Min height cannot exceed Max height.');
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={['#6C5CE7', '#74b9ff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
          ...
          { key: 'Age Range', value: `${ageMin} – ${ageMax}`, editor: (
            <View style={styles.pickerRow}>
              <Picker style={styles.picker} selectedValue={ageMin} onValueChange={setAgeMin}>
                {ages.map(a => <Picker.Item key={a} label={`${a}`} value={a} />)}
              </Picker>
              <Picker style={styles.picker} selectedValue={ageMax} onValueChange={setAgeMax}>
                {ages.map(a => <Picker.Item key={a} label={`${a}`} value={a} />)}
              </Picker>
            </View>
          )},
          { key: 'Distance', value: `${distance} km`, editor: (
            <Picker style={styles.picker} selectedValue={distance} onValueChange={setDistance}>
              {kmOptions.map(k => <Picker.Item key={k} label={`${k} km`} value={k} />)}
            </Picker>
          )},
          { key: 'Ethnicity', value: formatArrayValue(ethnicity), editor: (
            <MultiSelectRow options={ethnicOpts} value={ethnicity} onChange={setEthnicity} exclusiveOptions={['All']} />
          )},
          { key: 'Religion', value: formatArrayValue(religion), editor: (
            <MultiSelectRow options={relOpts} value={religion} onChange={setReligion} exclusiveOptions={['All','None']} />
          )},
          { key: 'Goals', value: goals, editor: (
            <Picker style={styles.picker} selectedValue={goals} onValueChange={setGoals}>
              {goalOpts.map(g => <Picker.Item key={g} label={g} value={g} />)}
            </Picker>
          )},
          { key: 'Height', value: `${heightMin} – ${heightMax}`, editor: (
            <View style={styles.pickerRow}>
      ...
      <View style={styles.bottomContainer}>
        <TouchableOpacity style={styles.submitBtn} onPress={handleDone}>
          <LinearGradient colors={['#6C5CE7', '#74b9ff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.submitGradient}>
