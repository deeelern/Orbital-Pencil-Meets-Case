...
export const handleLike = async (
  ...
  try {
    ...
    if (isMutualMatch) {
      ...
      console.log(
        "Step 2: Recording match and marking as seen for current user..."
      );
      await updateDoc(currentUserRef, {
      ...
      console.log(
        "No mutual match yet - waiting for the other user to like back"
      );
    }
  } catch (error) {
    ...
    if (error.code === "permission-denied") {
      alert(
        "Permission denied. Please check your Firestore rules and try again."
      );
    } else {
